...
class Ant(Insect):
    ...
    is_container = False
    # ADD CLASS ATTRIBUTES HERE
    blocks_path = True  # All Ants block path except NinjaAnt

    def __init__(self, health=1):
        self.is_damage_doubled = False  # Keep track of whether damage has been doubled
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            p_ant = place.ant   # The ant already in the place
            if p_ant.can_contain(self):     # Container is p_ant
                p_ant.ant_contained = self
            elif self.can_contain(p_ant):   # Container is self
                self.ant_contained = p_ant
                place.ant = self
            else:
...
class HarvesterAnt(Ant):
    ...
    name = 'Harvester'
    implemented = True
    food_cost = 2   # Overide `food_cost` of class `Ant`
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3   # Overide `food_cost` of class `Ant`
    lower_bound, upper_bound = 0, float('inf')  # Set the attacking distance range
    ...
    def nearest_bee(self):
        ...
        distance = 0
        while distance <= self.upper_bound: # It is impossible to attack bees in the places which are beyond `upper_bound`
            # Find bees
            if distance >= self.lower_bound and target_place.bees:
                return random_bee(target_place.bees)

            # Look up bees in the next place
            next_place = target_place.entrance
            if next_place and not next_place.is_hive:
                target_place = next_place
                distance += 1   # Increase distance between the ant and `target_place`
            else:
                return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        get_killed = False
        if amount >= self.health:   # If damage >= health, the ant gets killed
            get_killed = True

        # Apply reflect damage to bees in the same place
        bees = self.place.bees[:]
        reflect_damage = amount + (self.damage if get_killed else 0)
        for bee in bees:
            bee.reduce_health(reflect_damage)

        # Apply damage to the ant itself
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        # ContainerAnt can't contain another ContainerAnt or when it has already contained an ant
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants and does damage to bees meanwhile."""
    ...
    def action(self, gamestate):
        # Deal damage to all bees in the same place as TankAnt
        bees = self.place.bees[:]
        if bees:
            for bee in bees:
                bee.reduce_health(self.damage)

        # The contained ant performs action
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 12
        # QueenAnt can throw a leaf
        super().action(gamestate)

        # QueenAnt can doubles the damage of ants behind her
        place = self.place
        while True:
            place = place.exit
            if not place:   # The end of the tunnel
                break

            if place.ant:
                place.ant.double()  # Ant gets inspired
                if place.ant.is_container and place.ant.ant_contained:
                    place.ant.ant_contained.double()    # Ant protected by ContainerAnt also gets inspired
...
class SlowThrower(ThrowerAnt):
    ...
    food_cost = 6
    # BEGIN Problem EC 1
    damage = 0  # SlowThrower deal 0 damage
    implemented = True   # Change to True to view in the GUI
    # END Problem EC 1

    def throw_at(self, target):
        # BEGIN Problem EC 1
        # *** Function for overriding `action` ***
        def origin_action(self):    # Curry function `action` to simulate method `action`
            def f(gamestate):
        ...
        def slow_action(self):
            def f(gamestate):
                if not self.slow_remaining: # "Slow Down" debuff has been run out
                    self.action = origin_action(self)
                    self.action(gamestate) 
                else:
                    if gamestate.time % 2 == 0: # Bee should take action at even time
                        origin_action(self)(gamestate)
                    else:   # Bee should do nothing at odd time
                        pass
                    self.slow_remaining -= 1    # Count down
            return f
        # *** Function for overriding `action` ***
...
class ScaryThrower(ThrowerAnt):
    ...
    # BEGIN Problem EC 2
    implemented = True   # Change to True to view in the GUI
    scare_length = 2    # Times that scared bee tries to back away
    # END Problem EC 2
...
class NinjaAnt(Ant):
    ...
    food_cost = 5
    # OVERRIDE CLASS ATTRIBUTES HERE
    blocks_path = False # NinjaAnt doesn't block path
    # BEGIN Problem EC 3
    implemented = True   # Change to True to view in the GUI
    # END Problem EC 3
...
class LaserAnt(ThrowerAnt):
    ...
    # BEGIN Problem EC 4
    damage = 2
    battery_weaken = 0.0625
    distance_weaken = 0.25
    ...
    def insects_in_front(self):
        ...
        # Damage all Insects in front of LaserAnt (except itself and those in the hive)
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        weaken = self.battery_weaken * self.insects_shot + self.distance_weaken * distance
        damage = max(0, self.damage - weaken)
        return damage
        # END Problem EC 4
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            if not(self.scary_remaining and destination.is_hive):    # Scared Bee cannot move to hive
                self.move_to(destination)
            if self.scary_remaining:
                self.scary_remaining -= 1   # Count down
