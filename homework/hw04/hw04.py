def shuffle(s):
    ...
    for i in range(length // 2):
        shuffled += [first_half[i], second_half[i]]
...
def balanced(m):
    ...
    True
    """
    if is_planet(m):    # A planet is balanced
        return True
    l, r = left(m), right(m)
    flag = (length(l) * total_mass(end(l)) == length(r) * total_mass(end(r)))   # To see if torques are equal
    return flag and balanced(end(l)) and balanced(end(r))
