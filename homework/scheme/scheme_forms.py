...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        lambda_procedure = do_lambda_form(lambda_expr, env)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    formals = expressions.first
    body = expressions.rest # Construct the body which is a nested Scheme list of expressions
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            # When there are no corresponding sub-expressions
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        # Check that each binding has exactly 2 elements
        validate_form(binding, 2, 2)
        
        name, expr = binding.first, binding.rest.first
        val = scheme_eval(expr, env)
        # Build Scheme list of names and vals
        names = Pair(name, names)
        vals = Pair(val, vals)
        
        bindings = bindings.rest
    # Check whether each name is distinct
