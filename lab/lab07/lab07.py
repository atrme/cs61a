...
class FreeChecking(Account):
    ...
    def withdraw(self, amount):
        is_not_free = int(not bool(self.free_withdrawals)) 

        if self.balance < amount + (self.withdraw_fee * is_not_free):
            message = 'Insufficient funds'
        else:
            self.balance -= amount + (self.withdraw_fee * is_not_free)
...
def duplicate_link(s, val):
    ...
    if s.first == val:
        origin_rest = s.rest
        s.rest = Link(val, origin_rest)
    duplicate_link(next_s, val)
