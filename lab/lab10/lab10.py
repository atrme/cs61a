...
def eval_and(expressions):
    ...
    if expressions is nil:
        return scheme_t
    while True:
        subexp_eval = calc_eval(expressions.first)
        if subexp_eval is scheme_f:     # Short-curcuiting for `False` value
            return scheme_f
...
def eval_define(expressions):
    ...
    value = calc_eval(expressions.rest.first)   # Evaluate the expression to which `symbol` is bound to
    bindings[symbol] = value    # Bind `symbol` to `value`
    return symbol
